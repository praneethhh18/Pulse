...
- **🧪 Test foundation** — ✅ **BUILT**. Jest + ts-jest with a hermetic setup (scrubs credential env vars, forces demo mode, `TZ=UTC`); unit tests for the pure logic (time, resilience classifier, LLM heuristics), service tests (memory learning loop, context nudges + dismiss + tz), and a full-app `supertest` integration test. **32 tests green**, run on every push via **GitHub Actions CI** (`.github/workflows/ci.yml`). Files: `apps/api/test/`, `jest.config.js`. *(Next: add mobile RN tests.)*
